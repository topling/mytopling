...
void Rdb_io_watchdog::expire_io_callback(
    union sigval timer_data MY_ATTRIBUTE((__unused__))) {
  assert(timer_data.sival_ptr != nullptr);
...
int Rdb_io_watchdog::check_write_access(const std::string &dirname) const {
  ...
  const auto fname = myrocks::rdb_concat_paths(dirname, RDB_IO_DUMMY_FILE_NAME);
