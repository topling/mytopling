...
class Rdb_transaction {
 ...
 public:
  ...
  int finish_bulk_load(bool *is_critical_error = nullptr,
    ...
    options.allow_global_seqno = true; // topling specific
    options.write_global_seqno = false; // topling specific
    options.sync_file = false; // topling specific
...
class Rdb_transaction_impl : public Rdb_transaction {
  ...
  */
  ...
  void multi_get(rocksdb::ColumnFamilyHandle *const column_family,
    ...
    auto& ro = const_cast<rocksdb::ReadOptions&>(m_read_opts);
  ...
  rocksdb::Status get_for_update(const Rdb_key_def &key_descr,
    ...
    if (value != nullptr) {
      ...
      m_read_opts.just_check_key_exists = false;
    }
    else {
      m_read_opts.just_check_key_exists = true;
    }
