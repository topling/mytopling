  ...
  if (side_conf)
    g_repo.Put("sst_file_manager", {
      ...
      {"params", {
        ...
        {"max_trash_db_ratio", (double)rocksdb_max_trash_db_ratio_pct / 100},
      }}
...
if (side_conf && !repo_support_dynamic_create_cf()) {
  ...
  */
  ...
  if (side_conf) {
    bool Rdb_cf_options_update(rocksdb::ColumnFamilyOptions&, std::string_view);
...
int ha_rocksdb::index_next_with_direction_intern(uchar *const buf,
  ...
  for (;;) {
    ...
    if (likely(thd)) {
      ...
      if (unlikely(NoAtomic(thd->killed))) {
        rc = HA_ERR_QUERY_INTERRUPTED;
        break;
      }
      if (unlikely(cnt >= 200)) {
