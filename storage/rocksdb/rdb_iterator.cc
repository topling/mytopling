...
void Rdb_iterator_base::init(THD *thd, ha_rocksdb *rocksdb_handler,
  ...
  m_thd = thd;
  ((const Rdb_key_def**)(&m_thd))[-2] = &kd ; ROCKSDB_VERIFY_EQ(&m_kd , &kd );
  ((const Rdb_key_def**)(&m_thd))[-1] = &pkd; ROCKSDB_VERIFY_EQ(&m_pkd, &pkd);
  m_tbl_def = tbl_def;
...
void Rdb_iterator_base::setup_scan_iterator(
  ...
  if (!m_scan_it) {
    m_scan_it = rdb_tx_get_iterator(
        ...
        read_current, !read_current).release();
   #if defined(_MSC_VER) || defined(__clang__)
   #else
...
int Rdb_iterator_base::next_with_direction(bool move_forward, bool skip_next) {
  ...
  for (;;) {
    ...
    if (!m_ignore_killed && NoAtomic(m_thd->killed)) {
    ...
    if (unlikely(!is_valid_rdb_iterator(*m_scan_it))) {
...
int Rdb_iterator_base::get(Rdb_transaction* tx, const rocksdb::Slice *key,
                           rocksdb::PinnableSlice *value, Rdb_lock_type type,
                           bool skip_ttl_check, bool skip_wait) {
  m_valid = false;
  rocksdb::Status s =
    (type == RDB_LOCK_NONE) ? // avoid if..else copy Status s
    rdb_tx_get(tx, m_kd.get_cf(), *key, value, m_table_type) :
    rdb_tx_get_for_update(tx, m_kd, *key, value, m_table_type,
...
int Rdb_iterator_partial::seek_next_prefix(bool direction) {
  ...
  rc = Rdb_iterator_base::get(tx, &cur_prefix_key, &value, RDB_LOCK_NONE,
...
int Rdb_iterator_partial::handle_get_result(
  ...
  if (rc == HA_ERR_KEY_NOT_FOUND) {
    ...
    Rdb_transaction* tx = get_tx_from_thd(m_thd);
    rc = m_iterator_pk.get(tx, &pk_key, value, type, skip_ttl_check, skip_wait);
    if (rc) return rc;
...
int Rdb_iterator_partial::get(Rdb_transaction* tx, const rocksdb::Slice *key,
                              rocksdb::PinnableSlice *value, Rdb_lock_type type,
                              bool skip_ttl_check, bool skip_wait) {
  int rc = Rdb_iterator_base::get(tx, key, value, type, skip_ttl_check, skip_wait);
