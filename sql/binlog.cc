...
bool MYSQL_BIN_LOG::write_metadata_event(THD *thd,
  ...
  if (thd->should_write_hlc) {
    metadata_ev.set_hlc_time(thd->hlc_time_ns_next);
    /* HLC written, clear state */
    thd->should_write_hlc = false;
    write_event = wrote_hlc = true;
  } else {
    /* HLC was not assigned to this thd */
    thd->hlc_time_ns_next = 0;
  }

  // TTL read filtering time
  if (thd->binlog_ttl_read_filtering_ts) {
    metadata_ev.set_ttl_read_filtering_timestamp(
        thd->binlog_ttl_read_filtering_ts);
    write_event = true;
  }

  // TTL compaction time
  uint64_t compaction_ts = 0;
  if (!ha_update_binlog_ttl_compaction_ts(thd, &compaction_ts) &&
      compaction_ts != 0) {
    metadata_ev.set_ttl_compaction_timestamp(compaction_ts);
    write_event = true;
  }
  ...
  if (!result && wrote_hlc && thd->variables.response_attrs_contain_hlc &&
      tracker->is_enabled()) {
...
int MYSQL_BIN_LOG::init_index_file() {
  ...
  assert(apply_index_file_name_base != NULL);
...
void MYSQL_BIN_LOG::auto_purge() {
  // first run the auto purge validations
  if (check_auto_purge_conditions()) {
...
int MYSQL_BIN_LOG::finish_commit(THD *thd) {
  ...
  */
  ...
  assert(thd->commit_error != THD::CE_COMMIT_ERROR || !committed_low ||
         thd->commit_consensus_error);
  ...
  if (thd->hlc_time_ns_next != 0 && enable_binlog_hlc) {
    /* If this is a secondary, then update the local HLC to reflect the HLC of
     * this trx (as generated by primary). On the primary we skip this as it
     * would be redundant (since the HLC has already been updated when it was
     * assigned */
    if (thd->rli_slave || thd->rli_fake) {
      update_binlog_hlc(thd->hlc_time_ns_next);
    } else {
      // On the primary we just update the max HLC written in the binlog (on the
      // secondary this is done within the update_binlog_hlc() call)
      set_max_write_hlc_nsec(thd->hlc_time_ns_next);
    }
  }
  thd->hlc_time_ns_next = 0;
