...
void init_sql_command_flags() {
  ...
  */
  ...
  sql_command_flags[SQLCOM_SHOW_ENGINE_TRX] |= CF_ALLOW_PROTOCOL_PLUGIN;
...
static const char *deny_implicit_commit_if_db_read_only(THD *thd,
  ...
  for (Table_ref *table = all_tables; !db_name && table;
...
static int process_noncurrent_db_rw(THD *thd, Table_ref *all_tables) {
...
/* SHOW PROFILE begin */
  ...
  if (opt_log_ddl) {
    ...
    if (it != slow_log_ddls.end() && !log_throttle_ddl.log(thd, true)) {
      ...
      query_logger.slow_log_write(thd, output.c_str(), output.size(), false, 0,
                                  0);
...
int mysql_execute_command(THD *thd, bool first_level, ulonglong *last_timer) {
  ...
  if (unlikely(thd->slave_thread)) {
    ...
    */
    ...
    if (ret > 0) /* For all options other than ON */
    {
      ...
      query_logger.slow_log_write(thd, cross_db_query_log, log_len, false, 0,
                                  0);
  ...
  */
  ...
  switch (lex->sql_command) {
    ...
    case SQLCOM_BULK_LOAD_COMMIT:
    case SQLCOM_BULK_LOAD_ROLLBACK: {
      assert(lex->m_sql_cmd != nullptr);
      res = lex->m_sql_cmd->execute(thd);
      break;
    }
    ...
    case SQLCOM_CHANGE_DB: {
      if (!set_session_db_helper(thd, to_lex_cstring(query_block->db)))
        my_ok(thd);
...
bool parse_sql(THD *thd, Parser_state *parser_state,
  ...
  */
  if (parser_state->m_input.m_has_digest || sql_id_is_needed()) {
    /*
